...
public class ActivitiesAdapter extends BaseAdapter<ActivitiesAdapter.ViewHolder, Event> {
    ...
    class ViewHolder extends BaseViewHolder {
        ...
        void setActionAndDesc(UsageEvents.Event model) {
            String actionStr = null;
            SpannableStringBuilder descSpan = null;
            String fullName = model.getRepo() != null ? model.getRepo().getFullName() : null;
            AlwaysOnHotwordDetector.EventPayload.RefType refType = model.getPayload().getRefType();
            ...
            switch (model.getType()) {
                case CommitCommentEvent:
                    ...
                    break;
                case CreateEvent:
                    if (AlwaysOnHotwordDetector.EventPayload.RefType.repository.equals(refType)) {
                        actionStr = String.format(getString(R.string.created_repo), fullName);
                    } else if (AlwaysOnHotwordDetector.EventPayload.RefType.branch.equals(refType)) {
                        actionStr = String.format(getString(R.string.created_branch_at),
                                model.getPayload().getRef(), fullName);
                    } else if (AlwaysOnHotwordDetector.EventPayload.RefType.tag.equals(refType))  {
                        actionStr = String.format(getString(R.string.created_tag_at),
                                model.getPayload().getRef(), fullName);
                    }
                    break;
                case DeleteEvent:
                    if (AlwaysOnHotwordDetector.EventPayload.RefType.branch.equals(refType)) {
